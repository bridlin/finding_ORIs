...
printf "read directory is $alined_reads_dir\n"
...
for sample in "${input_list[@]}"; do
	macs2 callpeak  \
		...
		-c $alined_reads_dir/$sample\_control_F2R1_$bam_file_prefix\.bam  \
		-f BAMPE \
	...
	macs2 callpeak  \
		...
		-c $alined_reads_dir/$sample\_control_F1R2_$bam_file_prefix\.bam  \
		-f BAMPE \
...
for overlap in "${overlap_list[@]}" ; do for sample in "${input_list[@]}" ; do
	bedtools intersect \
		...
		-f 0.$overlap  \
		-F 0.$overlap  \
		-a $output_dir/peak_calling/$sample$minus_peak_file_ending \
		-b $output_dir/peak_calling/$sample$plus_peak_file_ending \
	...
	bedtools intersect \
		...
		-f 0.$overlap  \
		-F 0.$overlap  \
		-a $output_dir/peak_calling/$sample$plus_peak_file_ending \
		-b $output_dir/peak_calling/$sample$minus_peak_file_ending \
