...
echo "samples are ${input_list[@]}\n"
printf "aligned read directory is ${alined_reads_dir}\n"
printf "bamfile prefix is  ${bam_file_prefix}  \n"
echo -e "macs control is ${control_macs}\n"
echo -e "macs samples are ${input_list_macs[@]}\n"
...
echo -e "peak filtering\n"

for overlap in "${overlap_list[@]}" ; do for sample in "${input_list[@]}" ; do
	echo -e "filtering overlapping peaks for ${sample} with overlap ${overlap}\n" &&
	bedtools intersect \
		-wa  \
		-e  \
		-v \
